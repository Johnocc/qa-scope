-- =====================================================================
-- AXDB_v3.sql — 버전 스냅샷 (main 공유용)
-- 정본은 scripts/schema.sql (docker 초기화·npm run db:schema가 사용).
-- 이 파일은 v2 → v3 버전 이력 보존을 위한 동일 내용 사본이다.
--
-- v3 변경 요약 (vs AXDB_v2):
--   1. agents(상담사 마스터) 테이블 신설 + 더미 명부 7행 시드
--      (화면③ 대시보드·화면④ 개인 리포트의 상담사명·순위 공급원)
--   2. consultation_master.agent_id → agents FK 전환
--      (신규 DB: 인라인 FK / 기존 DB: 파일 하단 마이그레이션 블록이
--       명부 백필 후 조건부 ALTER — 재실행 안전)
--   3. 복합 인덱스 idx_master_agent_date (agent_id, consulted_at)
--      — 화면④ "상담사 × 기간" 집계용
--   4. app_config에 item_rate_warn(60)·item_rate_ok(80) 추가
--      — 화면④ 달성률 상태 컷 (API 계약 v1 §3.1, 하드코딩 금지)
-- 관련 문서: docs/api/화면4_상담사리포트_API계약_v1.md
-- =====================================================================
