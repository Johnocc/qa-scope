-- =====================================================================
-- AXDB_v4.sql — 버전 스냅샷 (main 공유용)
-- 정본은 scripts/schema.sql (docker 초기화·npm run db:schema가 사용).
-- 이 파일은 v3 → v4 버전 이력 보존을 위한 동일 내용 사본이다.
--
-- v4 변경 요약 (vs AXDB_v3):
--   1. 재채점 = 덮어쓰기 정책 반영 (팀 결정 2026-07-06) —
--      상담당 (consultation_id, evaluator) 정본 1건 유지.
--      evaluationRepo.saveFinalEvaluation()이 기존 정본을 삭제 후 새로 저장.
--   2. ai_evaluation_verify_log.stage ENUM에 '정본교체' 추가 —
--      폐기되는 정본의 원본 전문(ai_output_json)을 삭제 직전
--      candidate_json에 보존 (기존 DB는 하단 v4 마이그레이션 블록이 조건부 ALTER).
--   3. attempt_no 이어 세기 규약 명문화 — 재채점 시 1부터 재시작 금지
--      (UNIQUE 충돌 방지, verifyLogRepo.getNextAttemptNo() 사용).
-- =====================================================================
...
CREATE TABLE IF NOT EXISTS `ai_evaluation_verify_log` (
  ...
  `stage`           ENUM('형식검증','인용대조','교차검증','정본교체') NOT NULL
