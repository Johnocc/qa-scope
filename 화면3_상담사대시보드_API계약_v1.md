...
| 3 | **상단 "전체 평균" = 건 단위 평균** | 화면④의 `team_avg_score`와 같은 값·같은 수식 (`AVG(final_score)` — 상담사 평균의 평균 아님). 목업의 "건 단위 가중평균" 부제와 동일 의미 |
| 4 | **`weak_domain`은 화면④ §3.2와 동일 객체·동일 규칙** | 최저 달성률 영역, 동률 시 배점 합 큰 영역(D30>B26>C18>E14>A12), 최저 영역이 `item_rate_ok` 이상이면 `null`(약점 없음). `label` 매핑도 동일: A=`상담 도입` / B=`업무처리` / C=`공감·경청` / D=`불완전판매 고지` / E=`상담 마무리` ※ 목업의 "태도·공감" 표기는 이 매핑으로 교체 |
| 5 | **행 정렬 = 평균점수 내림차순** | 화면④ `summary.rank`(평균 내림차순 RANK)와 같은 순서 → ③의 행 순서와 ④의 순위가 일치. 동점은 같은 평균끼리 `agent_id` 오름차순(표시 안정성) |
...
| 필드 | 타입 | 카드 | 설명 |
|---|---|---|---|
| `avg_score` | number\|null | 전체 평균 | 기간 내 전체 평가 건의 건 단위 평균 (결정 3 — ④의 `team_avg_score`와 동일 값). 0건이면 `null` |
...
| `agent_count` | number | 상담사 수 | 기간 내 평가 1건 이상 보유 상담사 수 = `agents` 배열 길이 (④의 `agent_count`와 동일 값) |
...
| 값 | 화면① | 화면③ | 화면④ |
|---|---|---|---|
| 전체 평가 건수 | `summary.total_count` (기간 필터 없을 때) | `summary.evaluation_count` | `summary.total_evaluation_count` |
| 전체 위험 건수 | `summary.risk_count` (〃) | `summary.risk_count` | `summary.total_risk_count` |
| 전체 평균 | — | `summary.avg_score` | `summary.team_avg_score` |
| 상담사 건수·평균·위험·약점 | — | `agents[]` 각 필드 | `summary`의 대응 필드 |
...
- [ ] 목업 약점 라벨 "태도·공감" → 정본 매핑 `공감·경청`으로 교체 (결정 4)
