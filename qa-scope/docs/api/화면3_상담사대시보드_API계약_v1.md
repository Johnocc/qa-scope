...
| 5 | ★v1.2 **행 정렬 = ① 위험 건수 내림차순 → ② 평균점수 오름차순 → ③ agent_id 오름차순** | 순위 숫자·등수는 계속 계산·표시하지 않음 (변경 없음) — 화면③·④는 인사고과(순위 줄세우기)가 아니라 **코칭(약점 진단) 도구** (팀 결정 2026-07-08, ④ v1.2 결정 7과 공통). 상세 사유·null 처리·한계는 결정 5 부기(아래) 참조 |
...
**결정 5 부기 (2026-07-16 개정):**

개정 사유 (코칭 철학 정합화 — 2026-07-16 확정):
- 코칭 도구 철학 = 가장 문제 되는 건 우선 표시. 위험 플래그(불완전판매 의심)가 저점수보다 심각 → 최상단. 매니저의 실제 행동 순서("누구부터 볼까")와 일치
- 기존 점수 내림차순은 순위 숫자만 없을 뿐 사실상 등수표 — 철학과 충돌

null 평균 처리 (코드 정본): 같은 위험 건수 그룹 내에서 `avg_score: null`인 상담사는 뒤로 밀림. 둘 다 `null`이면 `agent_id` 오름차순. (§4 엣지 케이스상 표에는 평가 1건 이상 보유자만 오므로 실질 발생 없음 — 방어적 처리)

인지된 한계 + 확장 경로 (Q&A 방어):
- 위험 "건수"는 절대값이라 다건 상담사에게 불리할 수 있음
- 그러나 현 규모(상담사당 1~2건)에서 위험률은 0/50/100% 3개 값뿐이라 정렬 기준으로 기능하지 못하고, "1건 중 1건 = 100%" 왜곡이 노출됨 → 지금 전환 실익 없음 (John 승인)
- 전환 조건: 표본 충분 시(상담사당 n건 이상 축적) 위험률 정렬로 전환
...
**전체 평균 카드 — 유지 확정 (번복 이력, 2026-07-16):**
1. 07-16 오전: 카드 제거 확정 (코칭 행동으로 이어지지 않는다는 이유 — 코칭철학 결정기록 §2)
2. 07-16: 유지로 번복 — 최종 (팀 통계 기능 확장 고려. 전체 평균은 팀 단위 통계의 기초 수치라 제거 시 확장 방향과 충돌)

계약 영향 없음 — 결정 3(`summary.avg_score`)과 §3.2 필드, `verify:consistency` 검증 전부 현행 유지. 화면 표기도 유지 (카드 4개 구성 변경 없음).
관련 사실: 화면③ 하단 안내 문구("순위·팀 비교는 표시하지 않습니다")는 커밋 `5c0415a`에서 제거됨 — 팀 통계 방향 전환으로 문구 정합성 상실.

### 3.3 `agents` — 상담사 표 (결정 5 정렬, 평가 1건 이상 보유자만)
...
| v1.2 | 2026-07-16 | ① 결정 5 개정: 정렬 = 위험 건수↓ → 평균↑ → agent_id↑ (코칭 철학 정합화, 위험률 전환 조건 명시) ② 전체 평균 카드 제거 결정 번복 — 유지 확정 (팀 통계 확장 고려, 계약 필드 변경 없음) ③ 하단 안내 문구 제거 사실 기재 (5c0415a) |
